...
export default function Reports() {
  ...
  const generateComplianceReport = (entity: Entity): string => {
    return `
      <div style="font-family: Arial, sans-serif; max-width: 800px; margin: 0 auto; padding: 20px; background: #ffffff; color: #1a1a1a;">
        <h1 style="color: #1a1a1a; border-bottom: 3px solid #3b82f6; padding-bottom: 10px;">
          Compliance Report
        </h1>
        ...
        <h2 style="color: #1e293b; margin-top: 30px;">Entity Information</h2>
        <table style="width: 100%; border-collapse: collapse; margin: 20px 0;">
          <tr style="background: #f8fafc;">
            <td style="padding: 12px; border: 1px solid #e2e8f0; font-weight: bold; color: #334155;">Company Name</td>
            <td style="padding: 12px; border: 1px solid #e2e8f0; color: #1e293b;">${entity.name}</td>
          </tr>
          <tr style="background: #ffffff;">
            <td style="padding: 12px; border: 1px solid #e2e8f0; font-weight: bold; color: #334155;">Type</td>
            <td style="padding: 12px; border: 1px solid #e2e8f0; text-transform: capitalize; color: #1e293b;">${entity.type}</td>
          </tr>
          <tr style="background: #f8fafc;">
            <td style="padding: 12px; border: 1px solid #e2e8f0; font-weight: bold; color: #334155;">Status</td>
            <td style="padding: 12px; border: 1px solid #e2e8f0; color: #1e293b;">${entity.status}</td>
          </tr>
          <tr style="background: #ffffff;">
            <td style="padding: 12px; border: 1px solid #e2e8f0; font-weight: bold; color: #334155;">Compliance Status</td>
            <td style="padding: 12px; border: 1px solid #e2e8f0;">
              <strong style="color: ${entity.compliance === 'Pass' ? '#16a34a' : entity.compliance === 'Partial' ? '#ca8a04' : '#dc2626'};">
                ${entity.compliance}
              </strong>
            </td>
          </tr>
        ...
        <h2 style="color: #1e293b; margin-top: 30px;">Security & Compliance</h2>
        ...
        ${entity.documents && entity.documents.length > 0 ? `
          <h3 style="color: #1e293b; margin-top: 20px;">Document Checklist</h3>
          <ul style="color: #334155;">
            ${entity.documents.map(doc => `<li>${doc}</li>`).join('')}
          </ul>
        ` : ''}

        <div style="margin-top: 40px; padding: 20px; background: #f1f5f9; border-left: 4px solid #3b82f6;">
  ...
  const generateRiskReport = (entity: Entity): string => {
    return `
      <div style="font-family: Arial, sans-serif; max-width: 800px; margin: 0 auto; padding: 20px; background: #ffffff; color: #1a1a1a;">
        ...
        <h2 style="color: #1e293b; margin-top: 30px;">Entity Overview</h2>
        <table style="width: 100%; border-collapse: collapse; margin: 20px 0;">
          <tr style="background: #f8fafc;">
            <td style="padding: 12px; border: 1px solid #e2e8f0; font-weight: bold; color: #334155;">Company Name</td>
            <td style="padding: 12px; border: 1px solid #e2e8f0; color: #1e293b;">${entity.name}</td>
          ...
          <tr style="background: #ffffff;">
            <td style="padding: 12px; border: 1px solid #e2e8f0; font-weight: bold; color: #334155;">Owner</td>
            <td style="padding: 12px; border: 1px solid #e2e8f0; color: #1e293b;">${entity.owner}</td>
          </tr>
          <tr style="background: #f8fafc;">
            <td style="padding: 12px; border: 1px solid #e2e8f0; font-weight: bold; color: #334155;">Onboarded</td>
            <td style="padding: 12px; border: 1px solid #e2e8f0; color: #1e293b;">${new Date(entity.createdAt).toLocaleDateString()}</td>
          </tr>
        </table>

        <h2 style="color: #1e293b; margin-top: 30px;">Risk Assessment</h2>
        <div style="padding: 20px; background: ${entity.riskLevel === 'HIGH' ? '#fee2e2' : entity.riskLevel === 'MEDIUM' ? '#fef3c7' : '#dcfce7'}; border-radius: 8px; margin: 20px 0; border: 1px solid ${entity.riskLevel === 'HIGH' ? '#fca5a5' : entity.riskLevel === 'MEDIUM' ? '#fde047' : '#86efac'};">
          <h3 style="color: ${entity.riskLevel === 'HIGH' ? '#991b1b' : entity.riskLevel === 'MEDIUM' ? '#854d0e' : '#14532d'}; margin-top: 0;">
        ...
        <h2 style="color: #1e293b; margin-top: 30px;">Risk Factors</h2>
        <ul style="line-height: 2; color: #334155;">
          ${entity.hasPII ? '<li style="color: #991b1b;"><strong>High Risk:</strong> Handles PII/Sensitive Data</li>' : ''}
          ${!entity.hasControls ? '<li style="color: #854d0e;"><strong>Medium Risk:</strong> Missing security controls</li>' : ''}
          ${entity.documents && entity.documents.length < 3 ? '<li style="color: #854d0e;"><strong>Medium Risk:</strong> Incomplete documentation</li>' : ''}
          ${entity.hasControls && !entity.hasPII ? '<li style="color: #14532d;"><strong>Low Risk:</strong> Proper controls in place</li>' : ''}
        </ul>

        <div style="margin-top: 40px; padding: 20px; background: #f1f5f9; border-left: 4px solid #dc2626;">
          <p style="margin: 0; font-size: 14px; color: #475569;">
    ...
    return `
      <div style="font-family: Arial, sans-serif; max-width: 800px; margin: 0 auto; padding: 20px; background: #ffffff; color: #1a1a1a;">
        ...
        <table style="width: 100%; border-collapse: collapse; margin: 20px 0;">
          ...
          <tr style="background: #ffffff;">
            <td style="padding: 12px; border: 1px solid #e2e8f0; font-weight: bold; color: #334155;">Risk Level</td>
            <td style="padding: 12px; border: 1px solid #e2e8f0;">
        ...
        <h2 style="color: #1e293b; margin-top: 30px;">Audit Trail</h2>
        ${entityEvents.length > 0 ? `
          <table style="width: 100%; border-collapse: collapse; margin: 20px 0; border: 1px solid #e2e8f0;">
            <thead>
              <tr style="background: #3b82f6; color: white;">
                <th style="padding: 12px; text-align: left; border: 1px solid #2563eb;">Timestamp</th>
                <th style="padding: 12px; text-align: left; border: 1px solid #2563eb;">Action</th>
                <th style="padding: 12px; text-align: left; border: 1px solid #2563eb;">Details</th>
              </tr>
            </thead>
            <tbody>
              ${entityEvents.map((event, index) => `
                <tr style="background: ${index % 2 === 0 ? '#f8fafc' : '#ffffff'};">
                  <td style="padding: 12px; border: 1px solid #e2e8f0; font-size: 13px; color: #475569;">${new Date(event.timestamp).toLocaleString()}</td>
                  <td style="padding: 12px; border: 1px solid #e2e8f0; text-transform: capitalize; color: #1e293b;">${event.action.replace(/_/g, ' ')}</td>
                  <td style="padding: 12px; border: 1px solid #e2e8f0; color: #334155;">${event.details}</td>
                </tr>
              `).join('')}
            </tbody>
          </table>
        ` : '<p style="color: #64748b;">No audit events found for this entity.</p>'}

        <h2 style="color: #1e293b; margin-top: 30px;">Summary</h2>
        <ul style="line-height: 2; color: #334155;">
        ...
        <div style="margin-top: 40px; padding: 20px; background: #f1f5f9; border-left: 4px solid #7c3aed;">
          <p style="margin: 0; font-size: 14px; color: #475569;">
